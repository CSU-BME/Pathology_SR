...
class PerceptualLoss(nn.Module):
    ...
    def forward_2(self, x, gt):
        ...
        x0_features = self.vgg(x0)
        x1_features = self.vgg(x1)
        gt0_features = self.vgg(gt0.detach())
        gt_features = self.vgg(gt.detach())

        # calculate perceptual loss
        if self.perceptual_weight > 0:
            percep_loss = 0
            for k in x1_features.keys():
                percep_loss += self.criterion(
                    x1_features[k], gt_features[k]) * self.layer_weights[k]
            percep_loss0 = 0
            for k in x0_features.keys():
                percep_loss0 += self.criterion(
                    x0_features[k], gt0_features[k]) * self.layer_weights[k]

            percep_loss= ((1-self.fore_weight)*percep_loss +self.fore_weight*percep_loss0)  *self.perceptual_weight
        else:
            percep_loss = None
        ...
        if self.style_weight > 0:
            if self.vgg_style is not None:
                x0_features = self.vgg_style(x0)
                x1_features = self.vgg_style(x1)
                gt0_features = self.vgg_style(gt0.detach())
                gt_features = self.vgg_style(gt.detach())

            style_loss = 0
            for k in x1_features.keys():
                style_loss += self.criterion(
                    self._gram_mat(x1_features[k]),
                    self._gram_mat(
                        gt_features[k])) * self.layer_weights_style[k]
            style_loss0 = 0
            for k in x0_features.keys():
                style_loss0 += self.criterion(
                    self._gram_mat(x0_features[k]),
                    self._gram_mat(
                        gt0_features[k])) * self.layer_weights_style[k]

            style_loss =(style_loss+style_loss0) *0.5 * self.style_weight
        else:
            style_loss = None
