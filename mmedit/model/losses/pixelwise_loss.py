...
class MSELoss(nn.Module):
    ...
    def __init__(self, loss_weight=1.0, reduction='mean', fore_weight=0.1,sample_wise=False):
        super().__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. '
                             f'Supported ones are: {_reduction_modes}')
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        if isinstance(pred, tuple):
            target_low=target[:,:,0::2,0::2]
            return self.loss_weight * (self.fore_weight*mse_loss(
                pred[0],
                target_low,
                weight,
                reduction=self.reduction,
                sample_wise=self.sample_wise)+ (1-self.fore_weight)*mse_loss(
                pred[1],
                target,
                weight,
                reduction=self.reduction,
